...
struct dumper
{
    ...
    void output_string(Module::TextOffset offset)
    {
        ...
        if (index != -1)
        {
            ...
            const char* string = reinterpret_cast<const char*>(p + bits::rep(offset));
            ostr << string;
        }
        else
            ostr << "[invalid]" << bits::rep(offset);
    }
    ...
    void output_string_table()
    {
        ...
        {
            ostr << "[" << i << "]:";
            ostr << p + bits::rep(strings[i]) << std::endl;
        }
        ostr << "}\n";
    }
    ...
    void output_partition(const Module::PartitionSummaryData& summary)
    {
        ...
        */
        ...
        const std::byte* partition = base + bits::rep(summary.offset);
        ostr << "{\n";

        {
            ostr << std::setw(2) << std::setfill('0') << std::hex;
            auto block = partition;
            for (unsigned int i = 0; i < bits::rep(summary.cardinality); ++i)
            {
                for (unsigned int j = 0; j < bits::rep(summary.entry_size); ++j)
                {
                    ostr << std::to_integer<int>(block[j]) << " ";
                }
                ostr << std::endl;
                block += bits::rep(summary.entry_size);
            }
            ostr << std::dec;
        }
    ...
    */
    void output_partitions()
    {
        ...
        auto partition_summary_base = reinterpret_cast<const Module::PartitionSummaryData*>(base + bits::rep(header.toc));
        for (unsigned int i = 0; i < bits::rep(count); ++i)
        {
