...
namespace Module {
    ...
    enum class MonadicOperator : uint16_t {
        ...
        MsvcConfusedPopState,   // An EH state represented directly in a unary expression for invoking destructors after
                                // invoking a ctor.
        MsvcConfusedDtorAction, // An EH state represented directly in a unary expression for invoking destructors
                                // directly.
        MsvcConfusedVtorDisplacement,    // The compiler generated expression representing an offset amount to a virtual
                                         // base pointer address during initialization.
        MsvcConfusedDependentExpression, // At times the old YACC parser will 'give up' parsing a dependent expression
                                         // and simply return a constant with a dummy bit set. This operator attempts to
                                         // catch these offending dependent expression values.
    ...
    enum class DyadicOperator : uint16_t {
        ...
        Destruct,      // x.~T()
        DestructAt,    // p->~T()
        Cleanup,       //                      -- abstract machine, evaluate first operand then run second parameter as
                       //                      cleanup
    ...
    enum class TriadicOperator : uint16_t {
        ...
        MsvcConfusion = 0x0FE0, // FIXME: Anything below is confusion to be removed
        MsvcConfusedPushState,  // An EH state tree representing a call to a ctor, dtor, and msvc-specific EH state
                                // flags.
